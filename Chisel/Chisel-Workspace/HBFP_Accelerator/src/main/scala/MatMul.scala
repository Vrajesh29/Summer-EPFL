...
class SystolicArray(n: Int, m: Int, e: Int) extends Module{
    ...
    val myVerInputQ = Vec(n, Module(new Queue(new FP(m, e), n)).io)
    val myHorInputQ = Vec(n, Module(new Queue(new FP(m, e), n)).io)
    val myOutputQ = Vec(n, Module(new Queue(new FP(m, e), n)).io)

    val pes = Seq.fill(n, n)(Module(new PE(m, e)))

    for (i <- 0 until n) {
        myVerInputQ(i).enq <> io.in.ver(i)
        myHorInputQ(i).enq <> io.in.hor(i)
        myHorInputQ(i).deq.valid := true.B
        myVerInputQ(i).deq.valid := true.B
        pes(i)(0).io.in.hor := Mux(ShiftRegister(myHorInputQ(0).deq.fire, i), ShiftRegister(myHorInputQ(i).deq.bits, i), ShiftRegister(0.U.asTypeOf(new FP(m, e)), i))
        pes(0)(i).io.in.ver := Mux(ShiftRegister(myVerInputQ(0).deq.fire, i), ShiftRegister(myVerInputQ(i).deq.bits, i), ShiftRegister(0.U.asTypeOf(new FP(m, e)), i))
    }
    ...
    for (i <- 0 until n) {
        myOutputQ(i).enq <> pes(i)(n - 1).io.out.result
        io.out.result(i) <> myOutputQ(i).deq
    }
