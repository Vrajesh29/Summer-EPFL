...
class SystolicArray(n: Int, m: Int, e: Int) extends Module {
  ...
  // Connect inputs. One of the inputs need to be transposed to make the systolic array work.
  for (i <- 0 until n) {
    for (j <- 0 until n) {
      pes(i)(0).io.in.hor := ShiftRegister(io.in(i)(j).hor, i)
      pes(0)(i).io.in.ver := ShiftRegister(io.in(j)(i).ver, i)
    }
  }
