...
class HBFP_PE_MIXMAN(blockSize: Int, m: Int, e: Int) extends Module{
  val io = IO(new Bundle{
    ...
    adder.io.in.b.man <> result_buffer.man
    adder.io.in.b.sign <> result_buffer.sign
    adder.io.in.b.exp <> result_buffer.exp
...
// The below systolic array is good, but only for testing purposes. The real systolic array is after this one.
  class SystolicArray_HBFP_forTesting_MIXMAN(blockSize: Int, n: Int, m: Int, e: Int) extends Module {
    val io = IO(new Bundle {
        val in = Input(new Bundle {
  ...
  val pes = Seq.fill(n, n)(Module(new HBFP_PE_MIXMAN(blockSize, m, e)))
  ...
  for (i <- 0 until n) {
    myinputQ_hor(i).io.deq.ready := ShiftRegister(io.in.hor(i).valid, i + 1) // Shift changes based on the configuration. For normal case it will be i+1
  ...
  for (i <- 0 until n) {
    for (j <- 1 until n) {
      pes(i)(j).io.in.hor := ShiftRegister(pes(i)(j - 1).io.out.hor, 1) // Will change based on the configuration. For normal case it will be 1.
    }
  }
  ...
  for (i <- 1 until n) {
    for (j <- 0 until n) {
      pes(i)(j).io.in.ver := ShiftRegister(pes(i - 1)(j).io.out.ver, 1) // Will change based on the configuration. For normal case it will be 1.
    }
  }
  // Connect outputs
  for (i <- 0 until n) {
...
object SystolicArray_HBFP_forTesting_MIXMANMain extends App {
  emitVerilog(new SystolicArray_HBFP_forTesting_MIXMAN(4, 2, 6, 8))
}
...
class HBFP_MAC_MIXMAN(blockSize: Int, m: Int, e: Int, NumberOfMultipliers: Int, MultiplierWidth: Int) extends Module{
  val io = IO(new Bundle {
    val in = Input(new Bundle{
  ...
  if (m < MultiplierWidth){
    val padding = 0.U((MultiplierWidth - m).W)
    ...
    for (i <- 0 until blockSize){
      val multiplication = ((padding ## io.in.a.man(i)) * (padding ## io.in.b.man(i)))
    ...
    val padding = 0.U((m%MultiplierWidth).W)

    val exp_addition = ShiftRegister(io.in.a.exp +& io.in.b.exp, 3)  // Change the amount of shift required based on the configuration.
    io.out.result.exp := Mux(exp_addition > (1<<e).asUInt - 1.U, (1<<e).asUInt - 1.U, exp_addition)

    val dotProductRegs = Seq.fill(blockSize)(RegInit(0.U((m).W)))
    val multiplication = Seq.fill(blockSize)(RegInit(0.U((2 * m).W)))
    val two_complement_buffer = Seq.fill(blockSize)(RegInit(0.S((m + 1).W)))
    val signs = Seq.fill(blockSize)(RegInit(0.U(1.W)))

    for (i <- 0 until blockSize){
      for (j <- 0 until ((m+m%MultiplierWidth)/MultiplierWidth)){
        for (k <- 0 until ((m+m%MultiplierWidth)/MultiplierWidth)){
          if (((m+m%MultiplierWidth)/MultiplierWidth) < NumberOfMultipliers){
            multiplication(i) := multiplication(i) + (((padding ## io.in.a.man(i))((j+1)*MultiplierWidth-1, j*MultiplierWidth)) * ((padding ## io.in.b.man(i))((k+1)*MultiplierWidth-1, k*MultiplierWidth)) << (j+k)*MultiplierWidth)
          }
          else{
          }
        }
      ...
      dotProductRegs(i) := Mux(multiplication(i) > (2<<m-1).U - 1.U, (2<<m-1).U - 1.U, multiplication(i))
      signs(i) := io.in.a.sign(i) ^ io.in.b.sign(i)
      two_complement_buffer(i) := Mux(signs(i)===1.U, (~(0.U ## dotProductRegs(i)) + 1.U), (0.U ## dotProductRegs(i))).asSInt
    }
    val sum = two_complement_buffer.reduce(_ +& _)
