...
class HBFP_Adder(m: Int, e: Int) extends Module{
  ...
  // The below logic for finding the mantissa addition is doubly checked. So you may not worry about it.
  ...
  io.out.result.man := Mux(io.in.flag === 1.B, Mux(addition > (1 << m).asUInt - 1.U, (1 << m).asUInt - 1.U, addition), Mux(addition > (1<<4).asUInt - 1.U, (1<<4).asUInt - 1.U, addition)) // If the addition is greater than the maximum value of mantissa, then we saturate it to the maximum value of mantissa.
