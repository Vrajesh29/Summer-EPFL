...
class SystolicArray_HBFP_forTesting(blockSize: Int, n: Int, m: Int, e: Int) extends Module {
  ...
  for (i <- 0 until n) {
    ...
    if (i != 0) {
      ...
      myinputQ_hor(i).io.deq.ready := Mux(io.in.flag === 1.B, ShiftRegister(io.in.hor(0).fire,4*i + 1), ShiftRegister(io.in.hor(0).fire,i+1))
    // The ready of the input queue of column(i) is the RegNext of that of column(i-1) and so on.
      myinputQ_ver(i).io.deq.ready := Mux(io.in.flag === 1.B, ShiftRegister(io.in.ver(0).fire,4*i + 1), ShiftRegister(io.in.ver(0).fire,i+1))
    }
