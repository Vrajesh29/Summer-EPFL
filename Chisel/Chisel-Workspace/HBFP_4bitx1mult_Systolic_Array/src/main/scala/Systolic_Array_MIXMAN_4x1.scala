...
class SystolicArray_HBFP_forTesting(blockSize: Int, n: Int, m: Int, e: Int) extends Module {
  ...
  val myinputQ_hor = Seq.fill(n)(Module(new Queue(new HBFP(blockSize, m, e), n*n*n)))  // The queue depth needs to be increased to prevent overflow for the m =6 case.
  val myinputQ_ver = Seq.fill(n)(Module(new Queue(new HBFP(blockSize, m, e), n*n*n)))
  ...
  for (i <- 0 until n) {
    ...
    myinputQ_hor(i).io.deq.ready := Mux(io.in.flag === 1.B, ShiftRegister(io.in.hor(i).valid,(4*i + 1)), ShiftRegister(io.in.hor(i).valid,(i + 1)))
    // The ready of the input queue of column(i) is the RegNext of that of column(i-1) and so on.
    myinputQ_ver(i).io.deq.ready := Mux(io.in.flag === 1.B, ShiftRegister(io.in.ver(i).valid,(4*i + 1)), ShiftRegister(io.in.ver(i).valid,(i + 1)))
  ...
  for (i <- 0 until n) {
    for (j <- 1 until n) {
      pes(i)(j).io.in.hor := Mux(io.in.flag === 1.B, ShiftRegister(pes(i)(j-1).io.out.hor, 4), ShiftRegister(pes(i)(j-1).io.out.hor, 1)) // Pass the horizontal input to the next PE after x cycles.
    }
  }
  ...
  for (i <- 1 until n) {
    for (j <- 0 until n) {
      pes(i)(j).io.in.ver := Mux(io.in.flag === 1.B, ShiftRegister(pes(i-1)(j).io.out.ver, 4), ShiftRegister(pes(i-1)(j).io.out.ver, 1))// Pass the vertical input to the next PE after x cycles.
    }
  }
...
// TODO: Bypass the outputs in the output queues. Currently, the outputs are stored in the Array.
...
object SystolicArray_HBFP_forTestingMain extends App {
  emitVerilog(new SystolicArray_HBFP_forTesting(16, 16, 6, 8))
}