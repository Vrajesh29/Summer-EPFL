...
class HBFP_GEMMSpec extends AnyFlatSpec with ChiselScalatestTester {
  "mult" should "pass" in {
    ...
    dut.clock.step(9*n-3) // 9n-3.  In every four cycles, we will have a cycle where all the outputs will be correct. 9n-3 may not be the first of these instances.
